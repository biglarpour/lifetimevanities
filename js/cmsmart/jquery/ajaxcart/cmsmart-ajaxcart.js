...
jQuery(function($) {
	...
	$('body').append('<div id="ajaxcart" class="ajaxcart">' + ajopacity + '</div><div class="ajaxcartanimate"></div><a id="ajaxcartloading" class="progress-bar progress-bar-striped active"></a><div id="ajaxallct"><div id="ajaxcartmsgc" class="ajaxcartmsgc"></div></div>');
	...
	linkcart.mouseover(function(e) {
		...
		if(($("#ajaxcart").width() + linkcart.eq(0).offset().left) > $(window).width()){
			...
			$("#ajaxcart").css('left', (linkcart.eq(0).offset().left + ($(window).width() / 2 )) + 'px');
		}

		$("#ajaxcart").css('top', '100px');
		//$("#ajaxcart").fadeIn(300);
...
//			$("#ajaxcart").show();
	...
	//fly to cart effect that does nothing
	function tryFly(){
